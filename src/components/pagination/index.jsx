...
const Pagination = ({pgnInfo, currentPageIdx, setCurrentPageIdx, color}) => {
      ...
      for(let p=pgnInfo.startPage; p<=pgnInfo.endPage; p++) {
          paginationDOM.push(
              <li key={p} className={`page-item ${currentPageIdx === (p) ? "active" : ""}`} onClick={() => {
                  ...
                  <a className="page-link" onClick={()=>void(0)}>
                      {p}
                  </a>
              </li>
          )
  ...
  return (
      pgnInfo ?
      <div className={`pagination-wrap flex space-between ${color != undefined ? color : ""} mt-5`}>
