...
const ResumeChange = () => {
  ...
  const handleAgeCalculator = (yearSelect) =>
    new Date().getFullYear() - parseInt(yearSelect) + "歳";

  // body 데이터 수정
  const handleSelectChangeEvent = (e) => {
    const key = e.target.id
      .replaceAll(" dropdown-button-dark-example1", "")
      .replaceAll(" product-status-active", "");
    let value;
  ...
  const handleChangeImage = async (e, index) => {
    ...
    for (let i = 0; i < file.length; i++) {
      ...
      reader.onload = (res) =>
        setImage((prevState) => [...prevState, res.target.result]);
  ...
  const handleCareerChange = (e, idx) => {
    ...
    tempArr[idx] =
      id === "projectPeriodSelect" ? parseInt(e.target.value) : e.target.value;
  ...
  const handleAddSkills = (e) => {
    if (skillItem.temp.skillCodeSelect && skillItem.temp.careerCodeSelect) {
      if (
        skillItem.arr.filter(
          (skill) => skill.name === skillItem.temp.skillCodeSelect.skillName
        ).length > 0
      ) {
        alert("スキルが重複されました。");
        document.querySelector(".refTarget_select").value = null;
        document
          .querySelectorAll(".refTarget_radio")
      ...
      setBody({
        ...body,
        skillCodeSelect: [
          ...body.skillCodeSelect,
          skillItem.temp.skillCodeSelect.skill,
        ],
        careerCodeSelect: [
          ...body.careerCodeSelect,
          skillItem.temp.careerCodeSelect.careerCode,
        ],
      });
      setSkillItem({
        arr: [
  ...
  const handleSubmit = async () => {
    ...
    rsFilePhoto.length > 0
      ? rsFilePhoto.map((item) => formData.append("rsFilePhoto", item))
      : formData.append("rsFilePhoto", new File([], "photo.png"));
    rsFileDocument.length > 0
      ? rsFileDocument.map((item) => formData.append("rsFileDocument", item))
      : formData.append("rsFileDocument", new File([], "document.pdf"));

    //서버로 보내기
    axios
      .put("/api/resume/modify", formData, {
        headers: {
  ...
  const findOneDepth = (code, group) => {
    ...
    let filterOneDepth = group.filter(
      (dep1) =>
        dep1.child.filter((dep2) => Object.values(dep2).indexOf(code) != -1)
          .length === 1
    );
      ...
      .then((res) => {
        if (res.data.result.regiInfoDto) {
          ...
          setSkillList({
            ...
            category: Array.from(
              new Set(
                res.data.result.skillList.map((item) => item.skillCategory)
              )
            ),
          ...
          setBody({
            ...
            skillCodeSelect:
              res.data.result.regiInfoDto.skillCodeSelect.split(","),
            careerCodeSelect:
              res.data.result.regiInfoDto.careerCodeSelect.split(","),
            projectNameSelect:
              res.data.result.regiInfoDto.projectNameSelect.split(","),
            projectRoleSelect:
              res.data.result.regiInfoDto.projectRoleSelect.split(","),
            projectPeriodSelect: res.data.result.regiInfoDto.projectPeriodSelect
              .split(",")
              .map((item) => parseInt(item)),
            projectProcessSelect: [
              ...res.data.result.regiInfoDto.projectProcessSelect
                .split(";")
                .map((item) => item.split(",")),
            ],
          });
          ...
          if (mobile) {
            ...
            for (let i = 0; i < tempKeys.length; i++) {
              if (tempKeys[i].includes("Select")) {
                tempResult[tempKeys[i].replace("Select", "")] =
                  tempResult[tempKeys[i]];
            ...
            const businessDepthMenu = DepthSplit(
              mobile,
              "businessDepthMenu",
              "businessTypeList",
              "businessType"
            );
            const jobDepthMenu = DepthSplit(
              mobile,
              "jobDepthMenu",
              "jobTypeList",
              "jobType"
            );
            const hopeCareerDepthMenu = DepthSplit(
              mobile,
              "hopeCareerDepthMenu",
              "hopeCareerList",
              "hopeCareer"
            ...
            setMobileStatus({
              ...
              businessTypeOneDeps: findOneDepth(
                res.data.result.regiInfoDto.businessTypeSelect,
                businessDepthMenu
              ).businessType,
              hopeCareerOneDeps: findOneDepth(
                res.data.result.regiInfoDto.hopeCareerSelect,
                hopeCareerDepthMenu
              ).hopeCareer,
              jobTypeOneDeps: findOneDepth(
                res.data.result.regiInfoDto.jobTypeSelect,
                jobDepthMenu
              ).jobType,
    ...
    if (body && data) {
      ...
      for (let i = 0; i < body.skillCodeSelect.length; i++) {
        ...
        for (let j = 0; j < data.skillCareerList.length; j++) {
          if (
            data.skillCareerList[j].skillCareer === body.careerCodeSelect[i]
          ) {
  ...
  const handleChangeSKillList = () => {
    ...
    multipleSelectArr.map((multi, mKey) => {
      const multiName = multipleSkills.origin.filter(
        (skill) => skill.skill === multi.name
      )[0]?.skillName;
      const multiYear = data.skillCareerList.filter(
        (career) => career.skillCareer === multi.year
      )[0]?.skillCareerName;
      let passKey = -1;
    ...
    <>
      {isLoading ? (
        <div className="resume-mng ">
          <div className="box-type-default hidden lg:block">
            ...
            <div className="resume-regist-cont">
              ...
              <div className="form-flex-box flex space-between items-start">
                <div className="box-item flex flex-col">
                  ...
                  <MobileSelectBox
                    id={"userGender"}
                    value={body.userGenderSelect}
                    data={data && data.userGenderList}
                    onChange={handleSelectChangeEvent}
                    disabled={true}
                  />
                </div>
                <div className="box-item flex flex-col">
                  ...
                  <input
                    id="schoolNameSelect regular-form-1"
                    type="text"
              ...
              <div className="form-flex-box flex space-between items-start">
                <div className="box-item flex flex-col">
                  ...
                  <div className="flex items-center gap-2">
                    <MobileSelectBox
                      id={"userGender"}
                      value={body.userAgeSelect}
                      data={data && data.userAgeList}
                      onChange={handleSelectChangeEvent}
                      disabled={true}
                    />
                    <div className="btn btn-sm btn-ouline-secondary w-40 btn-age">
                      {handleAgeCalculator(
                        data.userAgeList.filter(
                          (year) => year.userAge === body.userAgeSelect
                        )[0].userAgeName
                      )}
                    </div>
                  </div>
                ...
                <div className="box-item flex flex-col">
                  ...
                  <input
                    id="majorNameSelect regular-form-1"
                    type="text"
                    className="form-control"
                    placeholder="専攻入力"
                    onChange={handleInputTextChangeEvent}
              ...
              <div className="form-flex-box flex space-between items-start">
                <div className="box-item flex flex-col">
                  ...
                  <SelectBox
                    id={"residentialAreaSelect"}
                    data={data && data.residentialAreaList}
                    onChange={handleSelectChangeEvent}
                    defaultValue={body.residentialAreaSelect}
                  />
                </div>
                <div className="box-item flex flex-col">
                  <div className="form-tit">
              ...
              <div className="form-flex-box flex space-between items-start">
                <div className="box-item flex flex-col">
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      id="userEmailSelect regular-form-1"
                      type="text"
                      className="form-control"
                      placeholder="イーメール入力"
                      onChange={handleInputTextChangeEvent}
                    ...
                    <div className="form-check form-switch flex gap-2">
                      <div className="switch-tit shrink-0 w40 text-center">
                        {body.userEmailFlag === "0" ? "非公開" : "公開"}
                      </div>
                      <input
                ...
                <div className="box-item flex flex-col">
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      id="phoneNumberSelect regular-form-1"
                      type="text"
                      className="form-control"
              ...
              <div className="form-flex-box flex space-between items-start">
                <div className="box-item2 flex flex-col">
                  ...
                  <button
                    className="btn btn-primary flex-start"
                    onClick={() => {
                  ...
                  >
                    {isLoading
                      ? data.hopeCareerList.filter(
                          (item) =>
                            item.hopeCareer ===
                            (body.hopeCareer
                              ? body.hopeCareer
                              : body.hopeCareerSelect)
                        )[0].hopeCareerName
                      : ""}
                  </button>
                </div>
              ...
              <div className="form-flex-box flex space-between items-start">
                ...
                <div className="box-item flex flex-col">
                  ...
                  >
                    {" "}
                    {isLoading
                      ? data.jobTypeList.filter(
                          (item) =>
                            item.jobType ===
                            (body.jobType ? body.jobType : body.jobTypeSelect)
                        )[0].jobTypeName
                      : ""}
                  </button>
                </div>
                <div className="box-item flex flex-col">
                  ...
                  >
                    ...
                    {isLoading
                      ? data.businessTypeList.filter(
                          (item) =>
                            item.businessType ===
                            (body.businessType
                              ? body.businessType
                              : body.businessTypeSelect)
                        )[0].businessTypeName
                      : ""}
                  </button>
                </div>
              ...
              <div className="flex-box2-cont form-flex-box">
                ...
                <div className="flex items-center space-between mb-4">
                  ...
                  <div
                    className="btn btn-sm btn-register w-40 btn-age"
                ...
                <div className="skil_list">
                  <div className="list-group list-over">
                    {skillList &&
                      skillList.skillName.map((skill, key) => {
                        return (
                          <div key={key} className="list-group-item">
                            <label className="checkbox-btn w-18">
                              <input
                                ...
                                type="radio"
                                onChange={(e) => handleUpdateSkill(e, skill)}
                              />
                              <span>
                                {skill.skillCategoryName} | {skill.skillName}
                              </span>
                            </label>
                          </div>
                        );
              ...
              <div className="blue-tit">登録されたスキルリスト</div>
                  ...
                  return (
                    <div key={key} className="blue-btn">
                      <span className="skilllist-langu blue-line pr-2 inline-block">
                        {
                          skillList.origin.filter(
                            (skill) => skill.skillName === item.name
                          )[0]?.skillCategoryName
                        }
                      </span>
                      ...
                      <button
                        ...
                        onClick={(e) => {
                          ...
                          console.log(tempArr);
                          tempArr.map((skill) => {
                            const code = skillList.origin.filter(
                              (origin) => origin.skillName === skill.name
                            )[0]?.skill;
                            const year = data.skillCareerList.filter(
                              (career) => career.skillCareerName === skill.year
                            )[0]?.skillCareer;
              ...
              <div className="flex-box2-tit flex space-between mt-16">
                ...
                <div className="text-slate-400">
                  {body.additionalCommentSelect === ""
                    ? "0"
                    : body.additionalCommentSelect.length}{" "}
                  / 200字
                </div>
              </div>
              <div className="flex-box2-cont textarea_style">
                <textarea
                  name=""
                  id="additionalCommentSelect"
                  cols=""
                  rows="10"
                  className="w-full"
                  maxLength={200}
                  placeholder="自由に自己紹介してください。（200字以内）"
                  onChange={handleInputTextChangeEvent}
              ...
              <div className="attach-wrap flex">
                ...
                <div className="flex flex-col attach-cont-wrap">
                  {fileNames.map((file, index) => {
                    return (
                      <div
                        className="attach-cont-item flex items-center space-between"
                        key={index}
                      >
                        <input
                          className="upload-name mr-2 attach-cont-tit"
                          value={file}
                          placeholder=""
                          readOnly
                        />
                        <button
                          className="attach-cont-btn"
            ...
            <div className='flex flex-col attach-cont-wrap'>
              ...
              <div className="download-btn flex">
                <button
                  className="btn btn-outline-primary flex items-center h48"
                  onClick={() =>
                    window.open("https://hitobito-net.com/api/files/sample.pdf")
                  }
                >
                  サンプル·ダウンロード
                ...
                <button
                  className="btn btn-primary flex items-center h48"
          ...
          {mobile.businessDepthMenu &&
          mobile.jobDepthMenu &&
          mobile.hopeCareerDepthMenu ? (
            <Modal
              show={modalFlag.main}
              onHidden={() => {
                setModalFlag({
                    ...
                    data={
                      ...
                      mobile[
                        ...
                        ? mobile[
                            ...
                            modalFlag.goal
                              ? depthMenu.hopeCareer.depth
                              : modalFlag.occupation
                              ? depthMenu.jobType.depth
                              : modalFlag.business
                              ? depthMenu.businessType.depth
                              : 0
                          ]?.child
                        : []
                    }
      ...
      >
        <ModalBody className="p-10 text-center">
          <div className="modal-tit mb-5">200文字まで入力できます。</div>
      ...
      >
        <ModalHeader>
          <div className="flex space-between items-center w-full">
            <h2 className="modal-tit">スキル検索</h2>
            <button
              className=""
              onClick={() => {
                setskillPlusModal(false);
        ...
        <ModalBody className="p-10 text-center">
                    ...
                    skillName: [
                      ...skillList.origin
                        .filter((item) =>
                          item.skillName
                            .toUpperCase()
                            .includes(skillNameRef.current.value.toUpperCase())
                        )
                        .filter(
                          (item) => item.skillCategory === e.target.value
                        ),
                    ],
                  });
                }}
              >
                {multipleSkills.category.map((item) => {
                  return (
                    <option value={item}>
                      {
                        multipleSkills.origin.filter(
                          (skill) => skill.skillCategory === item
                        )[0].skillCategoryName
                      }
                    </option>
                  );
            ...
            <div className="">
              <div className="flex gap-2">
                ...
                <div className="flex flex-col gap-2 items-start ">
                  <div className="form-tit">経歴期間</div>
                  <SelectBox
                    className={"refTarget_select w-40"}
                    id={"careerCode"}
                    data={data && data.skillCareerList}
                    onChange={handleUpdateMultipleSkill}
                    defaultValue={multipleSkills.selector?.careerCode}
                  />
                </div>
              </div>
              <div className="skil_list mt-4">
                <div className="list-group list-over">
                  {multipleSkills &&
                    multipleSkills.skillName.map((skill, key) => {
                      return (
                        <div key={key} className="list-group-item">
                          <label className="checkbox-btn w-18 flex item-center">
                            <input
                              ...
                              type="radio"
                              onChange={(e) =>
                                handleUpdateMultipleSkill(e, skill)
                              }
                            />
                            <span>
                              {skill.skillCategoryName} | {skill.skillName}
              ...
              {multipleSkills.selector.arr.map((item, key) => {
                return (
                  <div key={key} className="blue-btn">
                    ...
                    <span>
                      {
                        data.skillCareerList.filter(
                          (career) => career.skillCareer === item.year
                        )[0]?.skillCareerName
                      }
                    </span>
                    <button
                      ...
                      onClick={(e) => {
                        const tempArr = [
                          ...multipleSkills.selector.arr.filter(
                            (it) => it.name !== item.name
                          ),
                        ];
                        setMultipleSkills({
          ...
          <div className="w-full text-left mt-4 text-slate-500">
            * カテゴリー選択後、スキルを検索し経歴を入力してください。
          </div>
          {/* <div className="modal-subtit">管理者に問い合わせしてください。</div> */}
          <div className="flex flex-end gap-3 mt-16">
            ...
            >
              <img
                className="mr-2"
                src="/src/assets/images/add-btn.svg"
                alt=""
              />
              スキル追加
            </a>
