...
// 메세지함 reception 모바일 테이블
import MessageTableReceptionM from './message-table-reception-mobile';
...
function MessageReception() {
    ...
    // 클릭되었을때 모달에 들어갈 response중 item (메세지 상세보기 데이터)
    ...
    const getListMob = () => {
        ...
        axios.post("/api/app/msg/msgbox", listParm
            , {
    ...
    const getMsgSearchMob = () => {
        ...
        axios.post("/api/app/msg/msgbox/search", listParm, {
            withCredentials: true,
            headers: {
                accessToken: getCookie("accessToken"),
                lastLoginTime: getCookie("lastLoginTime"),
        ...
        if (msgSendTitle.length >= 201) {
            ...
            ServiceFetch('/msg/reply', 'post', {
                ...
                res.resultCode === "200" ? (
                    ...
                    setEditorData(""),
                    setMsgSendTitle(""),
                    !isMobile ? (msgState ? getMsgSearch() : getReceptionMsg()) : (msgState ? getMsgSearchMob() : getListMob())
                ) : res.resultCode === "301" ? (
                    setModalFail(true)
                ) : res.resultCode === "302" ? (
                    setMsgSendFail(true)
